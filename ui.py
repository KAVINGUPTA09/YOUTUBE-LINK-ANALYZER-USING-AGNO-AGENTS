                ...
                try:
                    ...
                    try:
                        ...
                        if not tables_df.empty:
                            ...
                            db_records = pd.read_sql_query(f"SELECT * FROM {table_name} ORDER BY rowid DESC LIMIT 5;", conn)
                            
                            st.markdown(f"**Database File:** `insighttube.db` | **Active Table:** `{table_name}`")
                            st.dataframe(db_records, use_container_width=True, height=220)
                        else:
                            st.info("SQLite database connected (`insighttube.db`). Initializing table records...")
                        conn.close()
                    except Exception as db_err:
                        st.caption(f"SQLite DB Inspector Note: {db_err}")
